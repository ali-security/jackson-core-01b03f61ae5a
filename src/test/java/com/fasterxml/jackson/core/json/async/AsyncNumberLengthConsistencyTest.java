...
public class AsyncNumberLengthConsistencyTest
    extends BaseTest
{
    // Default StreamReadConstraints.maxNumberLength; newStreamFactory() uses the default.
    ...
    private final JsonFactory STRICT_F = newStreamFactory();
    ...
    public void testIntegerPathStreamingChunksRejectsBeyondMaxNumberLength() throws Exception {
        JsonParser ap = STRICT_F.createNonBlockingByteArrayParser();
        try {
            ByteArrayFeeder feeder = (ByteArrayFeeder) ap.getNonBlockingInputFeeder();
            ...
            try {
                ...
                assertTrue("Unexpected message: " + msg, msg.contains("Number value length"));
                ...
                assertTrue("StreamConstraintsException raised too late: after " + chunksFed
     ...
     */
    public void testFractionPathStreamingChunksRejectsBeyondMaxNumberLength() throws Exception {
        JsonParser ap = STRICT_F.createNonBlockingByteArrayParser();
        try {
            ByteArrayFeeder feeder = (ByteArrayFeeder) ap.getNonBlockingInputFeeder();
    ...
    public void testIntegerPathJustUnderMaxNumberLengthParsesCleanly() throws Exception {
        ...
        try {
            ...
            assertEquals("Sub-limit integer should parse without StreamConstraintsException",
    ...
    public void testNegativeIntegerPathSmallChunksAccumulateRejectAtBoundary() throws Exception {
        ...
        try {
            ...
            try {
                ...
                assertTrue("StreamConstraintsException raised too early (sign likely counted): after "
                                + chunksFed + " chunks of " + smallChunk
                                + " (expected ~" + expectedFailChunk + ", maxNumberLength=" + MAX_NUM_LEN + ")",
                        chunksFed >= expectedFailChunk);
                assertTrue("StreamConstraintsException raised too late: after " + chunksFed
                                + " chunks of " + smallChunk + " (expected ~" + expectedFailChunk
                                + ", maxNumberLength=" + MAX_NUM_LEN + ")",
                        chunksFed <= expectedFailChunk + 1);
